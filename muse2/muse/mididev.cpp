...
bool MidiDevice::putEvent(const MidiPlayEvent& ev)
      ...
      if (ev.type() == ME_CONTROLLER) {
            ...
            if (a == CTRL_PROGRAM) {
                  ...
                  //if (!(MusEGlobal::song->mtype() == MT_GM && chn == 9)) {  // REMOVE Tim. Song type removal.
                        int hb = (b >> 16) & 0xff;
                        int lb = (b >> 8) & 0xff;
                        int pr = b & 0x7f;
                        if (hb != 0xff)
                  ...
                  //      }
                  //return false;   // Should absorb anyway and return, right?    p4.0.48 Tim.  // REMOVE Tim. Song type removal.
                  }
